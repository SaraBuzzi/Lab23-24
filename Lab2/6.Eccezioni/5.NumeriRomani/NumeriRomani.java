...
public class NumeriRomani {
    ...
    public static boolean isArab(String s) throws IllegalArgumentException {
        //EFFECTS: if s is an arab number returns true, false otherwise
        if (s.charAt(0) == '-') 
            throw new IllegalArgumentException("è negativo");
    ...
    public static String A2R(String s) throws IllegalArgumentException, MyArabNumberException {
        //EFFECTS: returns i converted to roman numbers (as a string)
        //      if s is not a number throw NumberFormatException
        //      if s is not a negative number throw IllegalArgumentException

        if (!isArab(s)) {
            throw new MyArabNumberException("non è convertibile in numero romano");
        }
    ...
    public static int R2A(String s) throws IllegalArgumentException {
        //EFFECTS: returns s converted to arab numbers (in int)
        if (!isRoman(s)) {
            throw new IllegalArgumentException("non è romano");
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                System.out.println(A2R(args[0]));
            } catch (MyArabNumberException | IllegalArgumentException f) {
